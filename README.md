...
└─────────────┘     └──────────────┘     └─────────┘
                           │
                    ┌──────┴──────┐
                    │ SiliconFlow │  LLM + Embedder + Reranker
                    └─────────────┘
```
...
# 构建并启动
docker build -t mem0xapi:0.1.13 .
...
services:
  mem0x:
    image: mem0xapi:0.1.13
    container_name: mem0xAPI
    ports:
      - "28768:28768"
    volumes:
      - /home/ubuntu/.mem0x/data:/app/data
      - /home/ubuntu/.mem0x/data/fastembed:/tmp/fastembed_cache
      - /home/ubuntu/.mem0x/config-compose.json:/app/config.json:ro
    environment:
...
| LLM | SiliconFlow Qwen2.5-14B | 事实提取（推理） |
...
```bash
# 导出镜像
sudo docker save mem0xapi:0.1.13 | gzip > /lhcos-data/mem0xapi-0.1.13.tar.gz

# 恢复镜像
gunzip -c /lhcos-data/mem0xapi-0.1.13.tar.gz | sudo docker load
```
